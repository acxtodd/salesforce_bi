    ...
    **Requirements: 1.1, 1.2, 1.3, 1.4, 1.5**
    ...
    def _query_salesforce_config(self, sobject: str) -> Optional[Dict[str, Any]]:
        ...
        **Requirements: 1.1, 1.2**
        ...
        """
        try:
            # Validate sobject name to prevent SOQL injection
            import re
            if not re.match(r'^[A-Za-z][A-Za-z0-9_]*$', sobject):
                LOGGER.warning(f"Invalid sobject name rejected: {sobject}")
                return None
    ...
    def _get_schema_cache(self):
        ...
        try:
            # Import schema cache from local schema_discovery module
