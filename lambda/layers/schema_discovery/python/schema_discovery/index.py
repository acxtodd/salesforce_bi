...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    **Requirements: 1.1, 1.6**
    ...
    5. get_export_fields - Get fields for batch export (for Apex integration)

    Event format:
    {
        "operation": "discover_all" | "discover_object" | "get_schema" | "invalidate_cache" | "get_export_fields",
        "sobject": "ascendix__Property__c",  // Required for discover_object, get_schema, get_export_fields
    ...
    try:
        ...
        elif operation == 'get_export_fields':
            if not sobject:
                return _error_response(400, "sobject is required for get_export_fields")
            return _handle_get_export_fields(cache, sobject)
...
def _handle_get_export_fields(cache: SchemaCache, sobject: str) -> Dict[str, Any]:
    """
    Get fields for batch export in a format suitable for Apex integration.

    **Task 34.1: Schema-Driven Export Integration**

    Returns fields organized for batch export:
    - text_fields: Fields to include in text chunks (for KB search)
    - filterable_fields: Picklist/boolean fields for filtering
    - relationship_fields: Lookup fields with related object info
    - graph_attributes: Fields to include as graph node attributes
    - has_record_types: Whether the object has RecordTypes (auto-include RecordType.Name)

    This enables AISearchBatchExport to read from Schema Cache instead of
    hardcoded IndexConfiguration__mdt, preventing schema drift.
    """
    schema = cache.get(sobject)
    ...
    if schema is None:
        return {
            ...
            'body': json.dumps({
                'success': False,
                'sobject': sobject,
                'message': f"Schema not found in cache for {sobject}. Run discover_object first."
            })
        }

    # Build export-friendly field lists
    text_fields = []
    ...
    relationship_fields = []
    for field in schema.relationships:
        relationship_fields.append({
            'name': field.name,
            'label': field.label,
            'related_to': field.related_to if hasattr(field, 'related_to') else None
        })

    # Graph attributes: fields that are good for filtering in graph queries
    # These include filterable fields (picklists) and key text fields (City, State)
    graph_attributes = []
    graph_attr_set = set()  # Avoid duplicates

    # Key text fields for graph filtering
    key_text_fields = ['ascendix__City__c', 'ascendix__State__c', 'Name']
    for field in schema.text:
        if field.name in key_text_fields:
            graph_attributes.append(field.name)
            graph_attr_set.add(field.name)

    # Key filterable fields for graph filtering
    key_filterable = ['RecordType', 'ascendix__PropertyClass__c', 'ascendix__BuildingStatus__c',
                      'ascendix__UseType__c', 'ascendix__PropertyType__c']
    for field in schema.filterable:
        if field.name in key_filterable and field.name not in graph_attr_set:
            graph_attributes.append(field.name)
            graph_attr_set.add(field.name)
        # Include RecordType.Name specifically for graph
        if field.name == 'RecordType' and 'RecordType.Name' not in graph_attr_set:
            graph_attributes.append('RecordType.Name')
            graph_attr_set.add('RecordType.Name')
    ...
    return {
        ...
        'body': json.dumps({
            ...
            'discovered_at': schema.discovered_at if hasattr(schema, 'discovered_at') else None,
            'field_counts': {
