...
class FieldSchema:
    ...
    **Requirements: 1.2, 1.3, 1.4, 1.5, 26.1-26.5**
    ...
    def to_dict(self, for_dynamodb: bool = False) -> Dict[str, Any]:
        """Convert to dictionary for serialization.

        Args:
            for_dynamodb: If True, convert floats to Decimal for DynamoDB compatibility.
                         If False (default), keep as float for JSON serialization.
        """
        result = {
            'name': self.name,
            'label': self.label,
    ...
    **Requirements: 1.1, 1.2, 1.3, 1.4, 1.5, 24.1-24.6**
    ...
    def to_dict(self, for_dynamodb: bool = False) -> Dict[str, Any]:
        ...
        result = {
            ...
            'label': self.label,
            'filterable': [f.to_dict(for_dynamodb=for_dynamodb) for f in self.filterable],
            'numeric': [f.to_dict(for_dynamodb=for_dynamodb) for f in self.numeric],
            'date': [f.to_dict(for_dynamodb=for_dynamodb) for f in self.date],
            'relationships': [f.to_dict(for_dynamodb=for_dynamodb) for f in self.relationships],
            'text': [f.to_dict(for_dynamodb=for_dynamodb) for f in self.text],
